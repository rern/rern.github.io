...
elif [[ $pkg_name == status ]]; then
	clear -x
	curl -sL https://github.com/rern/rAudio-status/archive/refs/heads/main.zip | bsdtar xf -
	mv rAudio-status{-main,}
	cd rAudio-status
	chmod +x compile.sh
	./compile.sh
	exit
...
buildPackage() {
	...
	case $name in
		mpd_oled | python-upnpp | xf86-video-fbturbo )
			bash <( curl -sL $https_io/github-download_dir.sh ) $https_io/PKGBUILD/0-+R/$name
			;;
   		* )
